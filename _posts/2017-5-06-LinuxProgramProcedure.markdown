...
> `hlt`指令一般情况下只是摆设，后面会提到，程序最后是不会执行到这个指令的，之所以放上这条指令，只是为了让汇编程序结构完整。
...
4. 在相关区域记录与程序安全运行相关的信息（这不是一个独立的步骤，而是遍及整个函数）。
5. 初始化`libc`。
...
可以看到，没有了`_start`函数，也就不会执行`__libc_start_main`。然而，`main`函数返回的值会存储在`__libc_start_main`指定的变量中，这个变量的地址通常是由寄存器来传递的。如果不指定入口函数，那个寄存器的值就为随机值（非法地址），所以产生了一个段错误。
